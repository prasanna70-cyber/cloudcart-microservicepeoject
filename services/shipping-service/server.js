...
const requests = new client.Counter({
  name: "shipping_service_http_requests_total",
...
app.post("/shipping/create", async (req, res) => {
  ...
  await pool.execute(
    ...
    [orderId, "CREATED", trackingNumber]
  );

  res.status(201).json({ orderId, status: "CREATED", trackingNumber });
});

app.get("/shipping/:orderId", async (req, res) => {
  const [rows] = await pool.execute("SELECT * FROM shipments WHERE order_id = ?", [req.params.orderId]);

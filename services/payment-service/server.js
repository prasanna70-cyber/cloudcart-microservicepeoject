...
const successCounter = new client.Counter({
  name: "payment_service_success_total",
  help: "Successful payments"
});

const failureCounter = new client.Counter({
  name: "payment_service_failed_total",
...
app.post("/payments/pay", async (req, res) => {
  ...
  const status = String(cardNumber) === "0000" ? "FAILED" : "SUCCESS";
  ...
  if (status === "SUCCESS") successCounter.inc();
  else failureCounter.inc();

  res.json({ orderId, amount, status, transactionId });
});

app.get("/payments/:orderId", async (req, res) => {
  const [rows] = await pool.execute("SELECT * FROM payments WHERE order_id = ? ORDER BY id DESC", [req.params.orderId]);
  res.json(rows);
});

...
app.post("/reviews", async (req, res) => {
  ...
  res.status(201).json({ id: result.insertId, productId, userId, rating, comment });
});

app.get("/reviews/product/:productId", async (req, res) => {
  const [rows] = await pool.execute("SELECT * FROM reviews WHERE product_id = ? ORDER BY id DESC", [req.params.productId]);
  res.json(rows);
});

...
app.get("/inventory/:productId", async (req, res) => {
  const [rows] = await pool.execute("SELECT * FROM inventory WHERE product_id = ?", [req.params.productId]);
  res.json(rows[0] || { product_id: Number(req.params.productId), quantity: 0 });
});
...
app.post("/inventory/reserve", async (req, res) => {
  ...
  if (result.affectedRows === 0) {
    return res.status(400).json({ error: "insufficient stock", productId, quantity });
  }

  res.json({ message: "inventory reserved", productId, quantity });
});

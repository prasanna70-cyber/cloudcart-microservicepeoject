...
app.post("/notifications/send", async (req, res) => {
  ...
  await pool.execute(
    ...
    [userId || null, type || "INFO", message || "Notification"]
  );

  console.log(`Notification: ${type} - ${message}`);

  res.status(201).json({ sent: true, type, message });
});

app.get("/notifications", async (req, res) => {
  const [rows] = await pool.execute("SELECT * FROM notifications ORDER BY id DESC LIMIT 50");
  res.json(rows);
});

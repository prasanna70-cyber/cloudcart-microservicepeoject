...
app.post("/orders", async (req, res) => {
  ...
  try {
    for (const item of items) {
      await axios.post(`${INVENTORY_URL}/inventory/reserve`, {
    ...
    const paymentResponse = await axios.post(`${PAYMENT_URL}/payments/pay`, {
      orderId,
      amount: total,
    ...
    if (payment.status === "SUCCESS") {
      ...
      const shippingResponse = await axios.post(`${SHIPPING_URL}/shipping/create`, { orderId });
...
app.get("/orders/:userId", async (req, res) => {
  const [rows] = await pool.execute("SELECT * FROM orders WHERE user_id = ? ORDER BY id DESC", [req.params.userId]);
  res.json(rows);
});

app.get("/orders/detail/:orderId", async (req, res) => {
  const [orders] = await pool.execute("SELECT * FROM orders WHERE id = ?", [req.params.orderId]);
  const [items] = await pool.execute("SELECT * FROM order_items WHERE order_id = ?", [req.params.orderId]);
